...
from src.generation.rag_mistral import RAGMistralGenerator
...
from src.web_scraping.web_scraping import WebScraper
...
def initialize_pipeline():
    ...
    try:
        mistral_key = st.session_state.get("mistral_api_key", "")
        ...
        if not all([mistral_key, assemblyai_key, firecrawl_key, zep_key]):
        ...
        with st.spinner("Initializing NotebookLM pipeline..."):
            ...
            podcast_script_generator = PodcastScriptGenerator(mistral_key) if mistral_key else None
            
            try:
                podcast_tts_generator = PodcastTTSGenerator() if mistral_key else None
        ...
        for uploaded_file in uploaded_files:
            try:
                ...
                if chunks:
                    ...
                    # Qdrant doesn't need explicit index creation
...
def render_api_keys_sidebar():
    with st.sidebar:
        ...
        with col2:
            st.markdown('<a href="https://console.mistral.ai/" target="_blank" style="text-decoration: none;"><button style="background: #4CAF50; color: white; border: none; padding: 5px 10px; border-radius: 3px; cursor: pointer;">Get API key</button></a>', unsafe_allow_html=True)
...
def generate_podcast(selected_source: str, podcast_style: str, podcast_length: str):
    if not st.session_state.pipeline or not st.session_state.pipeline['podcast_script_generator']:
        st.error("Podcast generation not available. Please check your Mistral API key.")
        return
        ...
        with st.spinner(f"📚 Gathering content from {selected_source}..."):
            try:
                ...
                search_results = pipeline['vector_db'].search(
                    ...
                    limit=50,
                    filter_dict={'source_file': selected_source}
                )
